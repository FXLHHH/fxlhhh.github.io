...
function handleExplodeAction(row, col, game, cellEl, tableEl) {
    ...
    panelEl.innerHTML = ` <h3>针不戳，你坚持了${game.seconds}秒</h3>`;
...
function renderWelcome() {
    ...
    <button id='advance1'>大师</button>
    </div>
    `;
    let buttonEl = gameEl.querySelector('button#level0');
    buttonEl.onclick = () => {
        let game = new Game(10, 10, 0.15);
        game.shuffle();
        renderTable(game);
    ...
    buttonEl.onclick = () => {
        let game = new Game(20, 20, 0.16);
        game.shuffle();
        renderTable(game);
    }
    buttonEl = gameEl.querySelector('button#advance1');
    buttonEl.onclick = () => {
        let game = new Game(25, 40, 0.18);
        game.shuffle();
        renderTable(game);
    }
}
